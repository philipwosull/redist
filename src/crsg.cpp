...
List crsg(List adj_list, NumericVector population, NumericVector area, NumericVector x_center,
          NumericVector y_center, int Ndistrict, double target_pop, double thresh,
          int maxiter) {
    ...
    NumericVector x = clone(x_center), y = clone(y_center), area_up = clone(area),
                  district_pop(Ndistrict);
    ...
    while (n_current > Ndistrict) {
        ...
        x[i_dist] = (x[i_dist] * area_up[i_dist] + x[j_dist] * area_up[j_dist]) /
                    (area_up[i_dist] + area_up[j_dist]);
        y[i_dist] = (y[i_dist] * area_up[i_dist] + y[j_dist] * area_up[j_dist]) /
                    (area_up[i_dist] + area_up[j_dist]);
    ...
    while ((max(district_pop) > max_pop) || (min(district_pop) < min_pop)) {
        ...
        // Rep code iterates over remaining ij districts if no swap available in optimal
        // district
        no_ij = TRUE;
        while (no_ij) {
            ...
            for (i = 0; i < swap_candidates.size(); i++) {
                dist[i] = dist_dist_diff(swap_candidates[i], i_dist, j_dist, x_center, y_center,
                                         x, y);
            }
        ...
        x[i_dist] = (x[i_dist] * area_up[i_dist] - x_center[prec_swap] * area[prec_swap]) /
                    (area_up[i_dist] - area[prec_swap]);
        y[i_dist] = (y[i_dist] * area_up[i_dist] - y_center[prec_swap] * area[prec_swap]) /
                    (area_up[i_dist] - area[prec_swap]);
        x[j_dist] = (x[j_dist] * area_up[j_dist] + x_center[prec_swap] * area[prec_swap]) /
                    (area_up[j_dist] + area[prec_swap]);
        y[j_dist] = (y[j_dist] * area_up[j_dist] + y_center[prec_swap] * area[prec_swap]) /
                    (area_up[j_dist] + area[prec_swap]);
        // Adjust Areas:
        area_up[i_dist] -= area[prec_swap];
        area_up[j_dist] += area[prec_swap];
