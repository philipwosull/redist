...
// [[Rcpp::export]]
List rsg(List adj_list, NumericVector population, int Ndistrict, double target_pop,
    ...
    IntegerVector idist_pmembers, jdist_pmembers, idist_pneighbors, idist_dneighbors,
        idist_newmembers;
    ...
    while (Ndist_current > Ndistrict) {
        ...
        if (idist_pmembers.size() > 1) {
            for (i = 1; i < idist_pmembers.size(); i++) {
                idist_pneighbors =
                    union_(idist_pneighbors, IntegerVector(adj_list[idist_pmembers(i)]));
            }
        } // end if
        ...
        // STEP 2B: Select one of district i's bordering districts at random and denote it as
        // district j
        ...
        for (i = 0; i < Nprecinct; i++) {
            if (member_dvec[i] == j_dist)
                member_dvec[i] = i_dist;
            if (member_dvec[i] > j_dist)
                member_dvec[i] = member_dvec[i] - 1;
        }
    ...
    // Only total this once --- we iteratively only add/subtract a single precinct from this
    // after each switch
    for (i = 0; i < Nprecinct; i++)
        district_pop[member_dvec[i]] += population[i];
    ...
    while ((max(district_pop) > maxpop) || (min(district_pop) < minpop)) {
        ...
        for (i = 0; i < p_neighbors_size; i++)
            d_neighbors[i] = member_dvec[p_neighbors[i]];
        ...
        for (i = (j_candidates.size() - 1); i > -1; i--) {
            if (j_candidates[i] == i_dist)
                j_candidates.erase(i);
        }
        ...
        if (j_candidates_size > 0) {
            ...
            for (i = 0; i < Nprecinct; i++)
                member_new_dvec[i] = member_dvec[i];
            ...
            if (countpartitions(alConnected) == Ndistrict) {
                ...
                // Assumed here that if idist_members.size==1 (a single precinct district), it
                // will never be the largest district
                idist_members = IntegerVector(member_plist[i_dist]);
                for (i = (idist_members.size() - 1); i > -1; i--) {
                    if (idist_members[i] == p)
                        idist_members.erase(i);
                }
