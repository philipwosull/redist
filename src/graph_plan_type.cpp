...
void GraphPlan::update_vertex_and_plan_specific_info_from_cut(
    ...
    assign_region_id_from_tree(ust_sampler.ust, region_ids, split_region1_tree_root,
                               split_region1_id, ust_sampler.vertex_queue);

    assign_region_id_from_tree(ust_sampler.ust, region_ids, split_region2_tree_root,
                               split_region2_id, ust_sampler.vertex_queue);
...
GraphPlan::get_valid_adj_regions_and_eff_log_boundary_lens(
...
double GraphPlan::get_log_eff_boundary_len(PlanMultigraph &plan_multigraph,
...
std::vector<double> get_ordered_tree_cut_devs(
    ...
    // REprintf("Starting at %d and %2.f and ", region_pop,
    // static_cast<double>(region_pop)*2.0);
    for (int i = 0; i < V; i++) {
        // ignore vertices not in the region
        if (i == root || (region_ids[i] != region_id1 && region_ids[i] != region_id2))
            continue;
        ...
        for (auto const cut_region1_size : smaller_cut_sizes_to_try) {
            ...
            // REprintf("Size 1=%d-Target %.2f, Size 2=%d-Target %.2f", cut_region1_size,
            // cut_region1_target, cut_region2_size, cut_region2_target); find the larger
            // deviation associated with assigning cut_region1_size to cutting below
            double max_below_dev =
                std::max(std::fabs(below_pop - cut_region1_target) / cut_region1_target,
                         std::fabs(above_pop - cut_region2_target) / cut_region2_target);
            // find the larger deviation associated with assigning cut_region1_size to cutting
            // above
            double max_above_dev =
                std::max(std::fabs(above_pop - cut_region1_target) / cut_region1_target,
            ...
            double min_dev = std::min(max_below_dev, max_above_dev);
...
void estimate_cut_k(const MapParams &map_params, const SplittingSchedule &splitting_schedule,
    ...
    for (int i = 0; i < N_max && idx < N_adapt; i++, idx++) {
        ...
        if (split_district_only) {
            ...
            // else get
            auto max_it =
                std::max_element(plan_ptrs_vec.at(i)->region_sizes.begin(),
                                 plan_ptrs_vec.at(i)->region_sizes.begin() + num_regions);
            biggest_region_id =
                std::distance(plan_ptrs_vec.at(i)->region_sizes.begin(), max_it);
            biggest_region_size = plan_ptrs_vec.at(i)->region_sizes[biggest_region_id];
        } else { // custom size
            biggest_region_size = -1;
            biggest_region_id = num_regions - 1;
        ...
        std::pair<int, int> min_and_max_possible_cut_sizes =
            splitting_schedule.all_regions_min_and_max_possible_cut_sizes[biggest_region_size];
        int min_possible_cut_size = min_and_max_possible_cut_sizes.first;
        int max_possible_cut_size = min_and_max_possible_cut_sizes.second;
