...
List cut_edges(List aList_con, double eprob) {
...
List make_swaps(List boundary_cc, List aList, NumericVector cds_old, NumericVector pop_vec,
                NumericVector cd_pop_vec, List constraints, CharacterVector psi_names,
                double minparity, double maxparity, double parity, int p, double eprob,
                double beta, const Graph &g) {
    ...
    for (int i = 0; i < p; i++) {
        ...
        while (boundary_cc.size() > 0) {
            ...
            List adjcheck_out =
                adjcheck_propcd(aList, prop_partitions, accepted_partitions, cds_prop);
            ...
            for (int j = 0; j < numcds_test; j++) {
                ...
                prop_cd_pops =
                    update_distpop(prop_partitions, pop_vec, prop_cd, curr_cd, cdspop_prop);
        ...
        mh_prob = update_mhprob(prop_partitions, aList, cds_prop, prop_cd, eprob, mh_prob);
