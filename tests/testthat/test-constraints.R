...
test_that("add_constr_status_quo errors without an existing plan", {
    map_no_ref <- suppressMessages(redist_map(iowa, ndists = 4, pop_tol = 0.05))
    expect_error(
...
test_that("add_constr_splits groups admin and stores counts", {
    ...
    el <- constr$splits[[1]]
    expect_length(el$admin, nrow(iowa_map))
    expect_length(unique(iowa_map$region), el$n)
...
test_that("add_constr_multisplits and total_splits mirror splits structure", {
    ...
    expect_length(unique(iowa_map$region), c1$multisplits[[1]]$n)
    expect_length(unique(iowa_map$region), c2$total_splits[[1]]$n)
